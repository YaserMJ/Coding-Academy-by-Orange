...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Ambika_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Ambika_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'ambika-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'ambika-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), AMBIKA_VERSION, true );
			wp_enqueue_style( 'ambika-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), AMBIKA_VERSION );

			wp_enqueue_script( 'ambika-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'ambika-typography-selectWoo' ), AMBIKA_VERSION, true );
			wp_enqueue_style( 'ambika-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), AMBIKA_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'ambika_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'ambika' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'ambika' );
			$this->json[ 'google_fonts' ] = apply_filters( 'ambika_typography_customize_list', ambika_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = ambika_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'ambika' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'ambika' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'ambika' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'ambika' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="ambika-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="ambika-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="ambika-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="ambika-font-transform">
					<label>
