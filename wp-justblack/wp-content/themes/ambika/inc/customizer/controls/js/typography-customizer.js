( function( api ) {
	...
	api.controlConstructor['ambika-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.ambika-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the ambika_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'ambika_get_all_google_fonts_ajax',
								ambika_customize_nonce: ambika_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.ambika-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.ambika-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.ambika-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.ambika-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.ambika-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.ambika-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.ambika-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.ambika-font-variant' );
			...
			control.container.on( 'change', '.ambika-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.ambika-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.ambika-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.ambika-font-family select' ).selectWoo();
	jQuery( '.ambika-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".ambika-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.ambika-font-variant' ).hide();
		}
	});
