...
add_action( 'after_setup_theme', 'ambika_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function ambika_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'ambika_construct_sidebars' );
}

if ( ! function_exists( 'ambika_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'ambika_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function ambika_woocommerce_start() { ?>
...
if ( ! function_exists( 'ambika_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'ambika_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function ambika_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'ambika_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'ambika_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function ambika_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'ambika_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'ambika_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'ambika_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function ambika_bbpress_css() {
...
if ( ! function_exists( 'ambika_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'ambika_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function ambika_buddypress_css() {
...
if ( ! function_exists( 'ambika_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'ambika_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function ambika_beaver_builder_css() {
