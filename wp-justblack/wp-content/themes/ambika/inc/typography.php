...
if ( ! function_exists( 'ambika_enqueue_google_fonts' ) ) {
	add_action( 'wp_enqueue_scripts', 'ambika_enqueue_google_fonts', 0 );
	/**
	 * Add Google Fonts to wp_head if needed.
	 *
	 */
	function ambika_enqueue_google_fonts() {
		...
		$not_google = str_replace( ' ', '+', ambika_typography_default_fonts() );
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				if ( ! isset( $ambika_settings[$key] ) ) {
					continue;
				}

				// If our value is still using the old format, fix it
				if ( strpos( $ambika_settings[$key], ':' ) !== false ) {
					$ambika_settings[$key] = current( explode( ':', $ambika_settings[$key] ) );
				}

				// Replace the spaces in the names with a plus
				$value = str_replace( ' ', '+', $ambika_settings[$key] );

				// Grab the variants using the plain name
				$variants = ambika_get_google_font_variants( $ambika_settings[$key], $key );
		...
		$google_fonts = apply_filters( 'ambika_typography_google_fonts', $google_fonts );

		// Get the subset
		$subset = apply_filters( 'ambika_fonts_subset','' );
		...
		$default_fonts = AMBIKA_DEFAULT_FONTS;

		// Create our URL using the arguments
		$fonts_url = add_query_arg( $font_args, '//fonts.googleapis.com/css' );

		// Enqueue our fonts
		wp_enqueue_style('ambika-default-fonts', $default_fonts, array(), null, 'all' );
		if ( $google_fonts ) {
			wp_enqueue_style('ambika-fonts', $fonts_url, array(), null, 'all' );
		}
	}
}

if ( ! function_exists( 'ambika_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'ambika_default_fonts_customize_register' );
	/**
	 * Build our Typography options
	 *
	 */
	function ambika_default_fonts_customize_register( $wp_customize ) {
		if ( function_exists( 'ambika_fonts_customize_register' ) ) {
			// Bail if Ambika Premium is active
			return;
		}
		...
		$defaults = ambika_get_default_fonts();

		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'Ambika_Typography_Customize_Control' );
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'ambika_sanitize_variants'
			)
		);

		$wp_customize->add_setting(
			'ambika_settings[body_font_weight]',
			array(
		...
		$wp_customize->add_setting(
			'ambika_settings[body_font_transform]',
			array(
		...
		$wp_customize->add_control(
			new Ambika_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'section' => 'font_section',
					'priority' => 1,
					'label' => __( 'Text Font', 'ambika' ),
					'settings' => array(
						'family' => 'ambika_settings[font_body]',
						'variant' => 'font_body_variants',
						'category' => 'font_body_category',
						'weight' => 'ambika_settings[body_font_weight]',
						'transform' => 'ambika_settings[body_font_transform]',
					),
				)
			)
		);
		...
		$wp_customize->add_setting(
			'ambika_settings[body_font_size]',
			array(
		...
		$wp_customize->add_control(
			new Ambika_Range_Slider_Control(
				...
				array(
					...
					'description' => __( 'Font size', 'ambika' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'ambika_settings[body_font_size]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 6,
		...
		$wp_customize->add_setting(
			'ambika_settings[body_line_height]',
			array(
				'default' => $defaults['body_line_height'],
				'type' => 'option',
				'sanitize_callback' => 'ambika_sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
		...
		$wp_customize->add_control(
			new Ambika_Range_Slider_Control(
				...
				array(
					'type' => 'ambika-range-slider',
					'description' => __( 'Line height', 'ambika' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'ambika_settings[body_line_height]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 1,
		...
		$wp_customize->add_setting(
			'ambika_settings[paragraph_margin]',
			array(
		...
		$wp_customize->add_control(
			new Ambika_Range_Slider_Control(
				...
				array(
					'type' => 'ambika-range-slider',
					'description' => __( 'Paragraph margin', 'ambika' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'ambika_settings[paragraph_margin]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 0,
...
if ( ! function_exists( 'ambika_get_all_google_fonts' ) ) {
	/**
	 ...
	 */
	function ambika_get_all_google_fonts( $amount = 'all' ) {
		...
		if ( apply_filters( 'ambika_alphabetize_google_fonts', true ) ) {
			asort( $fonts );
		}

		// Filter to allow us to modify the fonts array
		return apply_filters( 'ambika_google_fonts_array', $fonts );
	}
}

if ( ! function_exists( 'ambika_get_all_google_fonts_ajax' ) ) {
	add_action( 'wp_ajax_ambika_get_all_google_fonts_ajax', 'ambika_get_all_google_fonts_ajax' );
	/**
	 * Return an array of all of our Google Fonts.
	 *
	 *
	 */
	function ambika_get_all_google_fonts_ajax() {
		// Bail if the nonce doesn't check out
		if ( ! isset( $_POST[ 'ambika_customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'ambika_customize_nonce' ] ), 'ambika_customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'ambika_customize_nonce', 'ambika_customize_nonce' );
		...
		$fonts = ambika_get_all_google_fonts();
...
if ( ! function_exists( 'ambika_get_google_font_variants' ) ) {
	/**
	 * Wrapper function to find variants for chosen Google Fonts
	 * Example: ambika_get_google_font_variation( 'Open Sans' )
	 *
	 */
	function ambika_get_google_font_variants( $font, $key = '' ) {
		// Don't need variants if we're using a system font
		if ( in_array( $font, ambika_typography_default_fonts() ) ) {
			return;
		}
		...
		if ( $variants ) {
			...
			return implode(',', apply_filters( 'ambika_typography_variants', $output ) );
		}
	}
}

if ( ! function_exists( 'ambika_get_google_font_category' ) ) {
	/**
	 * Wrapper function to find the category for chosen Google Font
	 * Example: ambika_get_google_font_category( 'Open Sans' )
	 *
	 *
	 *
	 ...
	 */
	function ambika_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'ambika_get_font_family_css' ) ) {
	/**
	 ...
	 */
	function ambika_get_font_family_css( $font, $settings, $default ) {
		$ambika_settings = wp_parse_args(
		...
		$no_quotes = array(
			...
			apply_filters( 'ambika_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' )
		);

		// Get our font
		$font_family = $ambika_settings[ $font ];

		if ( 'System Stack' == $font_family ) {
			$font_family = apply_filters( 'ambika_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
		}
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . ambika_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'ambika_add_to_font_customizer_list' ) ) {
	add_filter( 'ambika_typography_customize_list', 'ambika_add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function ambika_add_to_font_customizer_list( $fonts ) {
		// Bail if we don't have our defaults
		if ( ! function_exists( 'ambika_get_default_fonts' ) ) {
			return;
		}
		...
		$all_fonts = ambika_get_all_google_fonts();
		$select_fonts = ambika_get_all_google_fonts( apply_filters( 'ambika_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going
			if ( ! isset( $ambika_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $ambika_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $ambika_settings[ $setting ], ambika_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $ambika_settings[ $setting ] ) ) ] = array(
				'name' => $ambika_settings[ $setting ],
		...
		if ( function_exists( 'ambika_secondary_nav_get_defaults' ) ) {
			...
			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], ambika_typography_default_fonts() ) ) {
